...
const baseUrl = 'https://nissanhub-qa.designory.com/suvs/pathfinder';
...
describe('Verify URLs for CTAs on Seating & Cargo', () => {
  ...
  it(`should visit ${link} and verify internal URLs`, () => {
    ...
    // Keep Exploring
      ...
      cy.wait(1000);
      cy.url().should('eq', 'https://nissanhub-qa.designory.com/suvs/pathfinder/nissan-pathfinder/index.html');
      cy.visit(fullUrl).get('[aria-label="Connectivity"]').click();
      cy.wait(1000);
      cy.url().should('eq', 'https://nissanhub-qa.designory.com/suvs/pathfinder/pathfinder-connectivity/index.html');
      cy.visit(fullUrl).get('[aria-label="Capability"]').click();
      cy.wait(1000);
      cy.url().should('eq', 'https://nissanhub-qa.designory.com/suvs/pathfinder/pathfinder-capability/index.html');
      cy.visit(fullUrl).get('[aria-label="Driver Assist & Safety Tech"]').click();
      cy.wait(1000);
      cy.url().should('eq', 'https://nissanhub-qa.designory.com/suvs/pathfinder/pathfinder-driver-assist/index.html');
    // Take the Next Step
      cy.visit(fullUrl);
